...
<body>
	<%
		...
		if (session.getAttribute("Account2")==null)
		{
			response.sendRedirect("amount-beneficiery.jsp");
		}
...
<form id="regForm"  name="myform" >
	...
	<!--=================================================
     ==== Money Transfer To New Beneficiery ====
    ==================================================-->
    ...
    <div class="tab" class="input-group mb-3" style="margin-left: 50px; margin-right: 50px;">
		<div id="label" class="alert alert-info alert-dismissible" style="height: 40px;">
  			<h4 style="margin-top: -6px;">Money Transfer To New Beneficiery</h4>
  		</div>
		...
		<div class="col-md-12">
			<div class="panel">
				<div class="panel-heading">
					<h4 class="lead" style="text-align: left; margin-bottom: 20px;">Paying From</h4>
					<div class="right">
						<button type="button" class="btn-toggle-collapse2"><i class="lnr lnr-chevron-up"></i></button>
					</div>
				</div>
				<div class="panel-body2 no-padding" style="display: block; overflow-x:auto; margin-top: -30px;">
					<table class="table table-striped">
						<thead>
							<tr>
								<th style="text-align: center; font-size: small;">A/c Number</th>
								...
								<th style="text-align: center; font-size: small;">Bank Name</th>
								<th style="text-align: center; font-size: small;">IFSC Code</th>
								<th style="text-align: center; font-size: small;">Ladger Amount</th>
							</tr>
						</thead>
						<%
							String acc_num2 = (String)session.getAttribute("Account2");
							String remark = (String)session.getAttribute("remark");
						
							double amount = Double.parseDouble(session.getAttribute("amount2").toString());
							
    								ResultSet rs2 = (ResultSet) con.createStatement().executeQuery("SELECT * FROM accounts WHERE acc_num='"+session.getAttribute("Account2")+"'");
    								if(rs2.next()){
    									String holder_name2 = rs2.getString("holder_name");
    									String account_no2 = rs2.getString("acc_num");

    									String close_bal = rs2.getString("closing_bal");
    									
    									double closing_bal = Double.parseDouble(close_bal);
    									System.out.println(holder_name2);
    									
    									if(amount <= max_bal){
    										if(amount <= closing_bal){
    											closing_bal = closing_bal - amount;
    										}
    									}
    					%>
						<tbody>
							<tr>
								<td><a href="#"><%=account_no2 %></a></td>
								<td><%=holder_name2 %></td>
								<td>-</td>
								<td>-</td>
								<td><span class="label label-success" style="font-size: 14px;"><%=closing_bal %></span></td>
							</tr>
						</tbody>
        ...
        <div class="form-row" style="margin-top: 20px;">
			<div class="form-group col-md-12">
				<h4 style="text-align: left; margin-left: -10px;">Payment Information</h4>
